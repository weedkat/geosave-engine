...
class GeosaveLightningModule(LightningModule):
    """Semantic-segmentation Lightning module — boilerplate catalog.

    ``num_classes`` defaults to ``len(LabelPipeline.schema)``.
    ``in_channels`` defaults to ``len(ImagePipeline.schema)``.
    Update pipeline schemas and these defaults are picked up automatically.
    ...
    def __init__(
        ...
        if in_channels is None:
            in_channels = len(ImagePipeline.schema)
        ...
        self.band_map = ImagePipeline.band_map()
