...
class GeosaveDataModule(GeoDataModule):
    ...
    Reads already-ingested layers only — never fetches or derives anything
    itself. Run ``geosave ingest`` (configs/ingest.yaml) first to populate
    ``root``. Keeping ingestion out of this class is deliberate: a model's
    raw-data needs and its training/serving dataloading are separate
    concerns (see configs/ingest.yaml vs configs/model.yaml).

    Args:
        root: Base directory. Split subdirs read from here (already ingested).
