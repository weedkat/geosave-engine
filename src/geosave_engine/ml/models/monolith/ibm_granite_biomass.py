    ...
    def forward_sliding(
        ...
        **sample_meta,
        ...
        def model_fn(patch: torch.Tensor) -> torch.Tensor:
            ...
            return self.forward_logits(patch_ctx).inputs['logits']

        return sliding_window_inference(
            model_fn, image, input_size, overlap_ratio, pad_size
        )

    @model_context(inputs=(['image'], ['logits']))
    def forward_logits(self, ctx: ModelContext) -> ModelContext:
        """Run biomass regression on a pre-normalized 6-band HLS S30 image.

        Args:
            ctx: ModelContext with ctx.inputs['image'] as (B, 6, H, W),
                normalized using ``img_mean`` / ``img_std``.

        Returns:
            ModelContext with ctx.inputs = {'logits': (B, 1, H, W)}.
        """
        output = self._model(ctx.inputs['image'])
        return ModelContext(
            inputs={'logits': output.output},
            sample_meta=ctx.sample_meta,
            metadata=ctx.metadata,
        )
