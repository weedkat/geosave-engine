"""Push a local LitDataStore folder to a Hugging Face Hub dataset repo.

Edit constants below, run directly. STORE_PATH must already be written
locally — LitDataStore also writes straight to s3://gs://r2:// directly,
...
from geosave_engine.geodata.datastore.litdata import LitDataStore

STORE_PATH = "data/train"  # local LitDataStore folder to push
CHUNK_SIZE = 1000  # must match how STORE_PATH was written — LitDataStore needs it even for a read-only open
...
def main() -> None:
    ...
    if CREATE_PARQUET:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            LitDataStore(STORE_PATH, chunk_size=CHUNK_SIZE).to_parquet(manifest_path)
            api.upload_file(
