...
class Place:
    ...
    def from_coordinate(cls, lat: float, lon: float) -> Place | None:
        ...
        Raises:
            ValueError: `lat` is outside [-90, 90] (see `validate_coordinate`).
        """
...
def reverse_geocode(lat: float, lon: float) -> dict | None:
    ...
    Returns None if Nominatim is unreachable, times out, or has no result
    for this coordinate — a real bug (missing geopy install, a programming
    error) still raises, not swallowed as a plain "no result".
    ...
    try:
        ...
        # geopy ships no type stubs — pyright infers kwargs from untyped defaults, both false positives.
        location = geolocator.reverse((lat, lon), timeout=5, language="en")  # pyright: ignore[reportArgumentType]
        if location is None:
            return None
        return location.raw.get("address", {})  # pyright: ignore[reportAttributeAccessIssue]
    except GeopyError:
        return None
