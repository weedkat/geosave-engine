...
def build_project(name, dir: str, template_dir):
    ...
    a training method, and a default model for the generated config. It copies the necessary
    ...
    selected_model = questionary.select(
        "Select the default model for config.yaml:",
    ...
    if generate_project(dir, name, task, method, selected_model, description, template_dir):
        # Inject selected model into config file.
        ...
        if config_path.exists():
            ...
            try:
                ...
                if "model" not in config_data or not isinstance(config_data["model"], dict):
                    config_data["model"] = {}
                config_data["model"]["name"] = selected_model
                with open(config_path, "w", encoding="utf-8") as f:
                    yaml.safe_dump(config_data, f, sort_keys=False)
                typer.secho(f"Injected selected model '{selected_model}' into config file.", fg=typer.colors.BLUE)
