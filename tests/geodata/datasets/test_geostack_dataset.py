"""Unit tests for GeoStackDataset: anchor-folder discovery, tensor rendering.
...
class TestDiscovery:
    ...
    def test_required_layers_excludes_incomplete_anchors(self, tmp_path):
        ...
        ds = GeoStackDataset(tmp_path, required_layers=["s2", "label"])
        assert len(ds) == 1
...
class TestRender:
    def test_stack_samples_batches_per_layer_key(self, tmp_path):
        ds = GeoStackDataset(_dataset_root(tmp_path), sel_bands={"s2": ["B02"]})
